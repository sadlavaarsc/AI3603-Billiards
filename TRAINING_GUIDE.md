...
`process_raw_match_data.py` 用于将原始对局数据转换为模型可训练的数据格式。该脚本具有以下特点：

- **递归遍历**：自动递归遍历指定目录下的所有子目录，找到所有 `match_*.json` 文件
- **流式处理**：采用流式写入方式，避免一次性将所有数据加载到内存，适合处理大量数据
- **内存优化**：使用生成器模式处理单个对局数据，只保留最近的状态信息
- **高效清理**：定期清理内存，防止内存泄漏

该脚本会将处理后的数据保存为包含状态、动作和价值标签的JSON格式训练数据。
